...
class LULCEnrichmentWrapper():
    ...
    def rasterize_vector_layers(self, year:int, save_osm_stressors:bool=False):
        ...
        with multiprocessing.Pool(self.max_threads) as pool:
            rasters_unbuff = pool.starmap(self.rasterize_vector_layer, [(self.lp.raster_metadata, self.vp.vector_refine, output_path ,0, lulc_code, layer_name) for layer_name,(output_path,lulc_code) in process_layers.items()])
